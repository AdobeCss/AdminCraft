...
import { GetAllPostos,CreatePostos,UpdatePostos,DeletePostos } from "./hooks";
...
import { GetAllUser } from "../Utilizadores/hooks";
...
type UserType = {
  ...
  userGroup: string;
  role: "APPROVER" | "VIEW ONLY" | "ENGINEER" | "ADMIN";
};


export type PostosType = {
  id?:number
 nome :string
 estado?:boolean
};
...
export default function UserManagement() {
  ...
  const { data, isLoading }= GetAllPostos()
  const { mutate } = DeletePostos()
  const [selectedUser, setSelectedUser] = useState<PostosType | null>(null);
  const [ErrorSearch, setErrorSearch] = useState(false);


  const handleRemove = (id: number) => {
    mutate(id as any|number)
  };
  ...
  return (
    <div className="flex flex-col w-full h-screen bg-zinc-100">
      <header className="flex bg-white max-md:px-6 px-4 items-center max-sm:justify-between max-sm:gap-2 max-md:justify-around justify-between border-b border-zinc-100 w-full h-[8%]">
        <div className="">
          <p className="text-[20px] max-sm:hidden max-md:text-[16px] font-bold ">Postos</p>
          <p className="text-zinc-400 max-md:hidden text-[11px]">todos os Perfil de Postos</p>
        </div>
        <div>
          <section className="flex h-[40px] max-md:w-[200px] w-[400px] px-3 p-1 rounded-md border items-center">
            ...
            <Input
              ...
              placeholder="pesquise os Perfil de Postos"
              className={` ${ErrorSearch?"text-red-700":"text-black"} outline-none outline-0 border-none font-semibold text-[14px] h-[30px] placeholder:text-zinc-300`}
            />
          </section>
        </div>
            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button className=''>
                  ...
                  <p className="">Adicionar Postos</p>
                </Button>
              </SheetTrigger>
              <SheetContent>
                <AddForm />
                    ...
                    <>
                      <TableHeader className="bg-gray-50 p-0">
                        <TableRow>
                          ...
                          <TableHead className="max-md:hidden">Opcoes</TableHead>
                          ...
                          >
                            ...
                            <TableCell className="px-6 py-4 max-md:hidden whitespace-nowrap text-right text-sm font-medium">
                              ...
                              <Sheet>
                                ...
                                <SheetContent>
                                  <EditForm
                                    role={selectedUser as PostosType}
                                  />
                                </SheetContent>
                              </Sheet>
...
function AddForm() {
  ...
  return (
    ...
    <form onSubmit={ form.handleSubmit( handleSubmit )} className="space-y-4">
      <SheetHeader>
        <SheetTitle>Adicionando Posto</SheetTitle>
      </SheetHeader>
      <div>
...
function EditForm({role}: { role: PostosType}) {

  const { mutate } = UpdatePostos()

  const [edited, setEdited] = useState<PostosType>(role);
  const form =useForm()
  ...
  return (
    <form onSubmit={form.handleSubmit( handleSubmit )} className="space-y-4">
      <SheetHeader>
        <SheetTitle>Editando Posto</SheetTitle>
      </SheetHeader>
      <div>
        <Label>Nome</Label>
        <Input
